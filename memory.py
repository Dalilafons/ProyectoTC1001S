...
from turtle import (
    up, goto, down, color, begin_fill, forward, left, end_fill, clear,
    shape, stamp, write, update, ontimer, setup, addshape, hideturtle,
    tracer, onscreenclick, done
)
from freegames import path

car = path('car.gif')  # Loads the car image to be used as background
# Creates a list of numbers 0-7, each appearing twice (16 total)
tiles = list(range(8)) * 2
state = {'mark': None}  # "Dictionary to track the selected tile"
hide = [True] * 16  # List tracking which tiles are hidden (all are hidden)
...
def square(x, y):
    ...
    # Draws a square by moving forward and turning left 4 times
...
def draw():
    ...
    if mark is not None and hide[mark]:  # If a tile is marked and it's hidden
        ...
        # Writes the tile's value centered
    ...
    # Writes the pairs counter
    write(f"Pairs found: {pairs_found} / 8", font=('Arial', 12, 'normal'))
    ...
    if game_complete:
        ...
        color('green')
        write("GAME COMPLETED!", font=('Arial', 18, 'bold'), align='center')
...
# Sets up the screen dimensions and position for 4x4 grid
setup(220, 220, 370, 0)
